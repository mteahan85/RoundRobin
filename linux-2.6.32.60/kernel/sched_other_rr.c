...
static void enqueue_task_other_rr(struct rq *rq, struct task_struct *p, int wakeup, bool b)
{
    ...
    //adds new process to end of queue. Put task to the end of the run list without the overhead of dequeue followed by enqueue.
 ...
 * moves current task to the end of the queue
 */
static void yield_task_other_rr(struct rq *rq)
{

  requeue_task_other_rr(rq, &rq->curr); //the first one may not need to be passed by reference
...
static struct task_struct *pick_next_task_other_rr(struct rq *rq)
{
  ...
  next = list_first_entry(queue, struct task_struck, other_rr_run_list);
...
static void task_tick_other_rr(struct rq *rq, struct task_struct *p,int queued)
{
  ...
  if(other_rr_time_slice == 0){ //holds cpu until complete because time quantum is 0
   return; 
  }else{
  	if(p->task_time_slice != 0){ //time slice is still going
		//decrement
		p->task_time_slice--;  	
  	   ...
  	   //moves it to the end of run queue
  		requeue_task_other_rr(rq, p);
  ...
  // not yet implemented -- need to figure out what int queued is supposed to be used for
}
